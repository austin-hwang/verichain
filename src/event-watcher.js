export function watchEvents(bidAuction, itemId, callback) {
  ...
  var bidEvent = bidAuction.BidCreated({
    fromBlock: "latest",
    toBlock: "latest",
    address: itemId
  });
  bidEvent.watch(function(error, result) {
    if (!error) {
      callback(
        "Bid received for Ticket Id " +
          result.args.pTicketId +
          " of amount " +
          result.args.bidAmount +
          " from address " +
          result.args.bidder,
        false,
        true
  ...
  var highEvent = bidAuction.HighestBid({
    fromBlock: "latest",
    toBlock: "latest",
    address: itemId
  });
  highEvent.watch(function(error, result) {
    if (!error) {
      callback(
        "Highest bid received for Ticket Id " +
          result.args.pTicketId +
          " of amount " +
          result.args.bidAmount +
          " from address " +
  ...
  var errEvent = bidAuction.BidError({
    fromBlock: "latest",
    toBlock: "latest",
    address: itemId
  });
  errEvent.watch(function(error, result) {
    if (!error) {
      callback(
        " Invalid Bid of amount " +
          result.args.bidAmount +
          " from address " +
          result.args.bidder +
          " <p> Error:  " +
          getErrMsg(result.args.errorCode.toString()),
        true,
        true
      );
    } else {
  ...
  var tktEvent = bidAuction.TicketAlloted({
    fromBlock: "latest",
    toBlock: "latest",
    address: itemId
  });
...
export function getErrMsg(errCode) {
  ...
  if (errCode === "100") {
    ...
    resMsg +=
      "Bidder has been alloted maximum allowed tickets for this Auction";
  } else if (errCode === "103") {
